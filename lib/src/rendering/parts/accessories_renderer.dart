...
final class AccessoriesRenderer implements AvatarPartRenderer {
  ...
  void render(AvatarRenderContext context, AvatarRenderState state) {
    ...
    state
      ...
      ..addLayer(
          ...
          meta: const {'part': 'faceMask'})
      ..addLayer('jewelry.dark', 186, jewelry.base, context.color('clothDark'),
          meta: const {'part': 'jewelry'})
      ..addLayer(
          'jewelry.accent', 187, jewelry.accent, context.color('fantasyLight'),
          meta: const {'part': 'jewelry'})
      ..addLayer('jewelry.light', 188, jewelry.light, context.color('white'),
          meta: const {'part': 'jewelry'});
  }
  ...
  _Headwear _headwear(AvatarRenderContext c) {
    ...
    final height =
        clampInt(c.integer('v4.headwearHeight') + coverage ~/ 2, 2, 18);
    ...
    if (style == 'baseballCap') {
      ...
      if (style == 'cowboyHat') {
        base.line(left, headTop + 1, left + 4, headTop - 1);
        base.line(right, headTop + 1, right - 4, headTop - 1);
      }
    ...
    light.data.setAll(
        ...
        light
            ...
            .intersect(base)
            .data);
    return _Headwear(base.removeSmallComponents(2, maxComponents: 5), shadow,
        light, accent.intersect(base));
  }
  ...
  _Eyewear _eyewear(AvatarRenderContext c, AvatarRenderState state) {
    ...
    if (style == 'monocleLeft' || style == 'monocleRight') {
      ...
      frame.line(4, eyeY - 5, 43, eyeY + 3);
        ...
        style == 'monoVisor' ||
        style == 'mirrorShades') {
      final visorHeight = clampInt(height, 1, 2);
      final visorWidth = clampInt(width, 3, 7);
      lens.fillRect(leftX - visorWidth ~/ 2, eyeY - visorHeight ~/ 2,
          rightX - leftX + visorWidth, visorHeight + 1);
      frame.data.setAll(0, lens.outline(diagonal: true).data);
      if (style == 'monoVisor')
      ...
      frame.line(
          leftX - width ~/ 2, eyeY, c.integer('head.leftX') - 1, eyeY - 1);
      frame.line(rightX + width ~/ 2, rightEyeY, c.integer('head.rightX') + 1,
          rightEyeY - 1);
    }
    if (style != 'rimless') {
    ...
    final renderedLens = darkLens
        // Ordered dithering reads as a checker/grid over the whole face at
        // 48px. Use a solid lens; clarity is more important than simulated
        // translucency at this resolution.
        ? clippedLens
        : tint == 0
            ? PixelMask()
  ...
  _FaceMask _faceMask(AvatarRenderContext c, AvatarRenderState state) {
    ...
    if (style == 'surgicalMask' ||
      ...
      base.data.setAll(
          0,
          head
              .intersect(maskFromPredicate((x, y) => x >= 24 && y >= noseY))
              .data);
    }
  ...
  _Jewelry _jewelry(AvatarRenderContext c) {
    ...
    if (neckStyle != 'none') {
      ...
      if (<String>['thinChain', 'thickChain', 'choker', 'beads']
        ...
        base
            .line(chainLeft, y, 24, y + 3, thickness: thickness)
        ...
        if (neckStyle == 'beads') {
          ...
          .contains(neckStyle)) {
        base.line(chainLeft, y, 24, y + 7).line(chainRight, y, 24, y + 7);
        if (neckStyle == 'dogTags') {
          ...
          accent.fillEllipse(
              24, y + 10, clampInt(size - 1, 1, 4), clampInt(size - 1, 1, 4));
          ...
          accent.fillEllipse(
              24, y + 9, clampInt(size, 1, 4), clampInt(size, 1, 4));
          light.set(23, y + 8);
        }
  ...
  _Cyber _cyber(AvatarRenderContext c, AvatarRenderState state) {
    ...
    if (style == 'cyberEyeLeft' || style == 'cyberEyeRight') {
      ...
      base.fillRect(c.integer('head.rightX') + 1, c.integer('ears.centerY') - 4,
          4 + coverage ~/ 2, 9);
