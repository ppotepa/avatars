...
final class ExtendedAtmosphereRenderer implements AvatarPartRenderer {
  ...
  void render(AvatarRenderContext context, AvatarRenderState state) {
    ...
    state
      ..addLayer('background.v42.dark', 2, scenic.dark,
          context.color('bgDark'), meta: const {'part': 'background'})
      ..addLayer('background.v42.light', 3, scenic.light,
      ...
      ..addLayer('cosmic.v42.dark', 3, cosmic.dark,
          context.color('fantasyDark'), meta: const {'part': 'cosmic'})
      ..addLayer('cosmic.v42.base', 4, cosmic.base,
          context.color('fantasyBase'), meta: const {'part': 'cosmic'})
      ..addLayer('cosmic.v42.light', 5, cosmic.light,
          context.color('fantasyLight'), meta: const {'part': 'cosmic'})
      ..addLayer('ambient.v42.dark', 5, ambient.dark,
          context.color('bgDark'), meta: const {'part': 'ambient'})
      ..addLayer('ambient.v42.light', 6, ambient.light,
          context.color('bgLight'), meta: const {'part': 'ambient'})
      ..addLayer('weather.v42.back.dark', 7, weatherBack.dark,
          context.color('bgDark'), meta: const {'part': 'weather'})
      ..addLayer('weather.v42.back', 8, weatherBack.base,
          context.color('fantasyBase'), meta: const {'part': 'weather'})
      ..addLayer('weather.v42.back.light', 9, weatherBack.light,
          context.color('fantasyLight'), meta: const {'part': 'weather'})
      ..addLayer('flames.v42.dark', 8, flames.dark,
          context.color('fantasyDark'), meta: const {'part': 'flames'})
      ..addLayer('flames.v42.base', 9, flames.base,
          context.color('fantasyBase'), meta: const {'part': 'flames'})
      ..addLayer('flames.v42.light', 9, flames.light,
          context.color('fantasyLight'), meta: const {'part': 'flames'})
      ..addLayer('backgroundEvent.v42.dark', 6, event.dark,
          context.color('bgDark'), meta: const {'part': 'backgroundEvent'})
      ..addLayer('backgroundEvent.v42.base', 7, event.base,
          context.color('clothAccent'), meta: const {'part': 'backgroundEvent'})
      ..addLayer('backgroundEvent.v42.light', 8, event.light,
          context.color('white'), meta: const {'part': 'backgroundEvent'})
  ...
  _TripleMask _scenic(AvatarRenderContext c) {
    ...
    if (style == 'sunrise' || style == 'sunsetMountains') {
    ...
    } else if (style == 'moonlitForest' || style == 'foggyForest' ||
        style == 'deadForest') {
      ...
      for (var i = 0; i < 4; i++) {
        ...
        accent.fillTriangle((x: x - 5, y: y + 1), (x: x + 5, y: y + 1),
            (x: x, y: y + 8));
      }
      ...
      if (style == 'warpTunnel') {
        for (var i = 0; i < 8; i++) light.line(24, 24, i * 7, i.isEven ? 0 : 47);
      }
  ...
  _TripleMask _cosmic(AvatarRenderContext c) {
    ...
    if (style == 'starsDense' || style == 'cosmicDust' ||
    ...
    if (style == 'constellation') {
      for (var i = 0; i < 6; i++) {
        ...
        light.set(x1, y1);
        if (i > 0) light.line(x1 - 7, 8 + positiveMod((i - 1) * 11, 20), x1, y1);
      }
    }
    if (style == 'auroraSky') {
      for (var x = 0; x < 48; x++) {
    ...
    if (style == 'asteroidField') {
      for (var i = 0; i < 7; i++) {
        dark.fillEllipse(rng.nextInt(2, 45), rng.nextInt(4, 35), 1 + i % 3, 1 + i % 2);
      }
    }
  ...
  _TripleMask _ambient(AvatarRenderContext c) {
    ...
    final dark = PixelMask();
    final light = PixelMask();
    if (style == 'none' || density == 0) return _TripleMask(dark, PixelMask(), light);
    final phase = c.rendering.animateBackground ? c.phase : 0;
    if (style == 'softFog' || style == 'deepFog' || style == 'dreamHaze' ||
        style == 'neonMist' || style == 'toxicCloud') {
      ...
      for (var x = -2; x < 52; x += 7) dark.fillEllipse(x, 7 + positiveMod(x, 5), 6, 3);
    } else if (style == 'heatHaze') {
      for (var x = 2; x < 47; x += 5) light.line(x, 47, x + positiveMod(phase, 3) - 1, 32);
    } else if (style == 'voidVeil') {
      dark.data.setAll(0, orderedDither(PixelMask.filled(), clampInt(density, 1, 6)).data);
    } else if (style == 'holyLight') {
      for (var x = 0; x < 48; x += 7) light.line(24, 0, x, 47);
    } else if (style == 'dustVeil') {
      light.data.setAll(0, orderedDither(PixelMask.filled(), clampInt(density, 1, 4), phase: phase).data);
  ...
  _TripleMask _weather(AvatarRenderContext c, {required bool back}) {
    ...
    for (var i = 0; i < count; i++) {
      ...
      } else if (style == 'embers' || style == 'sparks' || style == 'magicDust') {
  ...
  _TripleMask _event(AvatarRenderContext c) {
    ...
    if (style == 'lightningFlash' || style == 'alarmFlash' ||
        style == 'sunPulse') {
      light.data.setAll(0, orderedDither(PixelMask.filled(), 2 + intensity).data);
      ...
      if (style == 'eclipsePulse') dark.fillEllipse(38, 9, 3 + intensity, 3 + intensity);
    } else if (style == 'fireBurst' || style == 'lavaPulse') {
      for (var x = 0; x < 48; x += 5) {
        light.line(x, 47, x + 1, 30 - positiveMod(x + c.phase, 10), thickness: 2);
      }
    } else if (style == 'portalPulse') {
      final outer = PixelMask()..fillEllipse(24, 24, 18 + intensity, 22 + intensity);
      final inner = PixelMask()..fillEllipse(24, 24, 15 + intensity, 19 + intensity);
      ...
      dark.fillRect(x, 0, 8 + intensity, 48);
    }
    return _TripleMask(dark, base, light);
  }
}
...
final class ExtendedAdornmentRenderer implements AvatarPartRenderer {
  ...
  void render(AvatarRenderContext context, AvatarRenderState state) {
    ...
    state
      ..addLayer('backAdornment.v42.dark', 8, back.dark,
          context.color('outline'), meta: const {'part': 'backAdornment'})
      ..addLayer('backAdornment.v42', 9, back.base,
          context.color('clothBase'), meta: const {'part': 'backAdornment'})
      ..addLayer('backAdornment.v42.light', 9, back.light,
          context.color('fantasyLight'), meta: const {'part': 'backAdornment'})
      ..addLayer('symbols.v42.back', 9, symbols.back,
          context.color('fantasyBase'), meta: const {'part': 'symbols'})
      ..addLayer('halo.v42.back', 9, halo.back,
          context.color('fantasyBase'), meta: const {'part': 'halo'})
      ..addLayer('halo.v42.glow', 9, halo.glow,
          context.color('fantasyLight'), meta: const {'part': 'halo'})
      ..addLayer('headAdornment.v42.dark', 152, head.dark,
          context.color('outlineSoft'), meta: const {'part': 'headAdornment'})
      ..addLayer('headAdornment.v42', 153, head.base,
          context.color('clothAccent'), meta: const {'part': 'headAdornment'})
      ..addLayer('headAdornment.v42.light', 154, head.light,
          context.color('fantasyLight'), meta: const {'part': 'headAdornment'})
      ..addLayer('creature.v42.dark', 152, creature.dark,
          context.color('skinDeep'), meta: const {'part': 'creatureTrait'})
      ..addLayer('creature.v42', 153, creature.base,
          context.color('skinAccent'), meta: const {'part': 'creatureTrait'})
      ..addLayer('creature.v42.light', 154, creature.light,
          context.color('fantasyLight'), meta: const {'part': 'creatureTrait'})
      ..addLayer('hornAccent.v42', 152, hornAccent.base,
          context.color('fantasyDark'), meta: const {'part': 'fantasy'})
      ..addLayer('hornAccent.v42.light', 153, hornAccent.light,
          context.color('fantasyLight'), meta: const {'part': 'fantasy'})
      ..addLayer('halo.v42.front', 155, halo.front,
          context.color('fantasyLight'), meta: const {'part': 'halo'})
      ..addLayer('symbols.v42.front', 190, symbols.front,
          context.color('fantasyLight'), meta: const {'part': 'symbols'})
      ..addLayer('relic.v42.dark', 189, relic.dark,
          context.color('clothDark'), meta: const {'part': 'relic'})
      ..addLayer('relic.v42', 190, relic.base,
          context.color('clothAccent'), meta: const {'part': 'relic'})
      ..addLayer('relic.v42.light', 191, relic.light,
          context.color('fantasyLight'), meta: const {'part': 'relic'})
      ..addLayer('companion.v42.dark', 198, companion.dark,
          context.color('outline'), meta: const {'part': 'companion'})
      ..addLayer('companion.v42', 199, companion.base,
          context.color('clothAccent'), meta: const {'part': 'companion'})
      ..addLayer('companion.v42.light', 200, companion.light,
          context.color('fantasyLight'), meta: const {'part': 'companion'});
  }
  ...
  _HaloResult _halo(AvatarRenderContext c) {
    ...
    final centerY = clampInt(c.integer('head.topY') - 4 + c.integer('v4.haloHeight'), 1, 28);
    final tilt = c.integer('v4.haloTilt');
    final pulse = (c.string('v4.eventMotion') == 'haloPulse' ||
            c.string('v4.eventMotion') == 'haloOrbit')
        ? cyclicOffset(c.phase, animationPeriod(c.integer('v4.haloOrbitSpeed'), slow: 24, fast: 12), 1)
        : 0;
    final outer = PixelMask()
      ..fillEllipse(24 + tilt, centerY, size + pulse, clampInt(size ~/ 3 + pulse, 2, 9));
    ...
    if (style == 'doubleHalo') {
      final secondOuter = PixelMask()..fillEllipse(24 - tilt, centerY - 3, size - 2, size ~/ 3);
      final secondInner = PixelMask()..fillEllipse(24 - tilt, centerY - 3, size - 4, clampInt(size ~/ 3 - 1, 1, 8));
      ring = ring.union(secondOuter.subtract(secondInner));
    }
    if (style == 'brokenHalo' || c.integer('v4.haloBreakage') > 2) {
      ring = ring.subtract(maskFromPredicate((x, y) =>
          (x > 27 && y < centerY) || positiveMod(x + y, 9) == 0));
    }
    if (style == 'floatingSegments' || style == 'glitchHalo') {
      ring = ring.intersect(maskFromPredicate((x, y) => positiveMod(x + y + c.phase, 4) != 0));
    }
    if (style == 'runicHalo' || style == 'holySpikes' ||
        style == 'crownHalo' || style == 'thornHalo') {
      for (var i = 0; i < 10; i++) {
        final x = 8 + i * 4;
    ...
    if (style == 'flameHalo' || style == 'iceHalo' ||
        style == 'electricHalo' || style == 'cosmicHalo') {
      for (var x = 8; x < 41; x += 4) {
        ring.line(x, centerY, x + (x.isEven ? 1 : -1), centerY - 3 - positiveMod(x, 4));
      }
    }
    if (style == 'mechanicalHalo' || style == 'neonHalo') {
      for (var x = 9; x < 40; x += 6) ring.fillRect(x, centerY - 1, 3, 3);
    }
    final headBand = maskFromPredicate((x, y) => y <= c.integer('head.topY') + 1);
    ...
    if (glowAmount > 0) {
      var expanded = ring;
      for (var i = 0; i < clampInt(glowAmount, 1, 3); i++) expanded = expanded.dilated();
      glow.data.setAll(0, expanded.subtract(ring).intersect(
          maskFromPredicate((x, y) => positiveMod(x + y, 2) == 0)).data);
    }
  ...
  _SymbolResult _symbols(AvatarRenderContext c) {
    ...
    if (style == 'magicCircle' || style == 'clockworkRing' || style == 'spiral') {
    ...
    for (var i = 0; i < count; i++) {
      ...
      if (style == 'runes' || style == 'glyphs' || style == 'prayerText') {
        ...
        front.set(x - 1, y).set(x + 1, y).hLine(x - 1, x + 1, y + 1).set(x, y + 2);
      } else if (style == 'chains' || style == 'thorns') {
        back.line(x, y - 3, x + 3, y + 3);
      } else if (style == 'warningTriangles') {
        back.fillTriangle((x: x - 2, y: y + 2), (x: x + 2, y: y + 2), (x: x, y: y - 2));
  ...
  _TripleMask _backAdornment(AvatarRenderContext c) {
    ...
    if (style == 'bannerBack' || style == 'prayerScrollBack') {
      ...
      base.line(12, 47, 19, 21, thickness: 3).line(36, 47, 29, 21, thickness: 3);
    } else if (style == 'energyBackpack' || style == 'jetpackSmall' ||
        style == 'jetpackLarge') {
    ...
    } else if (style == 'capeTorn' || style == 'capeRoyal' ||
        style == 'cloakStarry') {
      base.fillTriangle((x: 8, y: 29), (x: 40, y: 29), (x: 24, y: 48));
      if (style == 'capeTorn') base.data.setAll(0, base.subtract(
          maskFromPredicate((x, y) => y > 40 && positiveMod(x + y, 5) == 0)).data);
      if (style == 'cloakStarry') {
        for (var i = 0; i < 12; i++) light.set(11 + positiveMod(i * 7, 27), 31 + positiveMod(i * 5, 15));
      }
      ...
      if (style == 'wingsHologram' || style == 'wingsGhostly') {
        base.data.setAll(0, orderedDither(base, style == 'wingsGhostly' ? 3 : 5).data);
      }
    }
  ...
  _TripleMask _headDetails(AvatarRenderContext c, AvatarRenderState state) {
    ...
    if (style == 'foreheadGem' || style == 'tiaraGem') {
      ...
      base.data.setAll(0, base.subtract(PixelMask()..fillEllipse(26, eyeY - 6, 3, 3)).data);
    } else if (style == 'sunDisc') {
      base.fillEllipse(24, eyeY - 5, 2, 2);
      base.hLine(19, 29, eyeY - 5).vLine(24, eyeY - 10, eyeY);
    } else if (style == 'sigil' || style == 'runeStrip' || style == 'crackGlow') {
      ...
      for (var x = 15; x < 34; x += 3) base.fillEllipse(x, eyeY - 7 - positiveMod(x, 3), 2, 1);
    ...
    if (side != 'none') {
      ...
      for (final direction in const <int>[-1, 1]) {
        final x = direction < 0 ? c.integer('head.leftX') - 1 : c.integer('head.rightX') + 1;
        if (side == 'finFrill' || side == 'leafEarsAccent') {
          base.fillTriangle((x: x, y: y - 4), (x: x + direction * 7, y: y), (x: x, y: y + 4));
        } else if (side == 'featherTuft' || side == 'furTufts') {
          base.fillTriangle((x: x, y: y - 3), (x: x + direction * 5, y: y - 7), (x: x + direction * 2, y: y + 1));
        } else if (side == 'gillSlits') {
          for (var i = 0; i < 3; i++) dark.line(x, y - 2 + i * 2, x + direction * 4, y - 1 + i * 2);
        } else if (side == 'sideSpikes' || side == 'smallHorns') {
          base.fillTriangle((x: x, y: y - 3), (x: x + direction * 6, y: y - 5), (x: x, y: y));
          base.fillTriangle((x: x, y: y + 1), (x: x + direction * 5, y: y + 5), (x: x, y: y + 4));
        } else if (side == 'antennaBulb' || side == 'antennaFeather') {
          base.line(x, y - 3, x + direction * 4, y - 10);
          light.fillEllipse(x + direction * 4, y - 10, 1, side == 'antennaFeather' ? 3 : 1);
        } else if (side == 'mechanicalPort' || side == 'audioReceiver' || side == 'orbitalNode') {
          base.fillRect(x + (direction < 0 ? -3 : 0), y - 3, 4, 7);
          light.set(x + direction, y);
        }
      }
    }
    final clip = state.mask('head').dilated(iterations: 2).union(state.mask('ears').dilated());
    return _TripleMask(dark.intersect(clip), base.intersect(clip), light.intersect(clip));
  }
  ...
  _TripleMask _creature(AvatarRenderContext c, AvatarRenderState state) {
    ...
    if (style == 'fangs') {
      light.fillTriangle((x: 21, y: mouthY), (x: 23, y: mouthY), (x: 22, y: mouthY + 3));
      light.fillTriangle((x: 25, y: mouthY), (x: 27, y: mouthY), (x: 26, y: mouthY + 3));
    } else if (style == 'tusks') {
    ...
    } else if (style == 'snoutHint' || style == 'catNose' || style == 'beakHint') {
      dark.fillEllipse(24, noseY, style == 'beakHint' ? 3 : 2, 1);
      if (style == 'beakHint') base.fillTriangle((x: 20, y: noseY), (x: 28, y: noseY), (x: 24, y: noseY + 4));
    } else if (style == 'gills') {
      for (var i = 0; i < 3; i++) {
        dark.line(14, noseY + i * 2, 18, noseY + 1 + i * 2);
        dark.line(34, noseY + i * 2, 30, noseY + 1 + i * 2);
      }
    } else if (style == 'scales' || style == 'furPatches' ||
        style == 'featherCheeks' || style == 'stoneSkin' ||
        style == 'barkSkin') {
      final area = state.mask('faceInner').subtract(state.mask('eyeSafety')).subtract(state.mask('mouthSafety'));
      base.data.setAll(0, orderedDither(area, style == 'stoneSkin' ? 5 : 3).data);
    } else if (style == 'glowVeins' || style == 'voidCracks') {
      light.line(15, c.integer('face.eyeY'), 20, noseY + 3)
          .line(33, c.integer('face.eyeY') + 1, 28, mouthY + 2);
      if (style == 'voidCracks') dark.data.setAll(0, light.data);
    } else if (style == 'slimeDroplets') {
      for (var x = 15; x < 34; x += 5) base.line(x, c.integer('head.bottomY') - 3, x, c.integer('head.bottomY') + positiveMod(x, 4));
    } else if (style == 'crystalGrowth') {
      for (var x = 14; x <= 34; x += 5) base.fillTriangle((x: x - 2, y: noseY + 8), (x: x + 2, y: noseY + 8), (x: x, y: noseY + 2 - positiveMod(x, 4)));
  ...
  _TripleMask _relic(AvatarRenderContext c) {
    ...
    if (style == 'multipleChains' || style == 'prayerBeads') {
      for (var offset = 0; offset < 3; offset++) {
        base.line(15 + offset, y, 24, y + 5 + offset)
      ...
      if (style == 'crystalPendant') {
        base.fillTriangle((x: 20, y: y + 8), (x: 28, y: y + 8), (x: 24, y: y + 15));
      }
    }
  ...
  _TripleMask _companion(AvatarRenderContext c) {
    ...
    final x = side < 0 ? 7 : 40;
    final y = 31;
    if (style == 'owl' || style == 'crow' || style == 'raven' || style == 'bat') {
      base.fillEllipse(x, y - 7, 4, 5);
      base.fillEllipse(x, y - 12, 3, 3);
      if (style == 'bat') {
        base.fillTriangle((x: x, y: y - 8), (x: x - side * 8, y: y - 13), (x: x - side * 4, y: y - 4));
      } else {
        base.fillTriangle((x: x - 3, y: y - 13), (x: x, y: y - 12), (x: x - 2, y: y - 16));
        base.fillTriangle((x: x, y: y - 12), (x: x + 3, y: y - 13), (x: x + 2, y: y - 16));
      }
      light.set(x - 1, y - 12).set(x + 1, y - 12);
    } else if (style == 'snake') {
      base.line(x, y, x + side * 5, y - 8, thickness: 2)
      ...
      light.fillTriangle((x: x - 2, y: y - 9), (x: x + 2, y: y - 9), (x: x, y: y - 14));
  ...
  _DoubleMask _hornAccent(AvatarRenderContext c, AvatarRenderState state) {
    ...
    if (style.contains('crystal') || style.contains('neon') ||
        style.contains('ice') || style.contains('mechanical')) {
      light.data.setAll(0, horns.intersect(
          maskFromPredicate((x, y) => positiveMod(x + y, 3) == 0)).data);
    }
    if (style == 'brokenLeft') {
      base.data.setAll(0, horns.intersect(maskFromPredicate((x, y) => x < 24 && y < c.integer('head.topY') - 2)).data);
    } else if (style == 'brokenRight') {
      base.data.setAll(0, horns.intersect(maskFromPredicate((x, y) => x > 24 && y < c.integer('head.topY') - 2)).data);
    } else if (style.contains('branching') || style.contains('Antlers') ||
        style == 'mooseFlat') {
      for (final direction in const <int>[-1, 1]) {
        final x = 24 + direction * 8;
        base.line(x, c.integer('head.topY'), x + direction * 7, c.integer('head.topY') - 8);
...
final class ExpressionRenderer implements AvatarPartRenderer {
  ...
  void render(AvatarRenderContext c, AvatarRenderState state) {
    ...
    final mouthStyle = _animatedMouth(c, _effectiveMouth(c, expression),
        faceAnimation, mouthMotion);
    ...
    final browClear = state.mask('foreheadZone').intersect(maskFromPredicate(
        (x, y) => y >= c.integer('face.eyeY') - 7 && y <= c.integer('face.eyeY') - 1));

    state
      ..addLayer('expression.mouth.clear', 113, mouthClear,
          c.color('skinBase'), meta: const {'part': 'expression'})
      ..addLayer('expression.mouth.dark', 114, mouth.dark,
          c.color('mouthDark'), meta: const {'part': 'expression'})
      ..addLayer('expression.mouth.base', 115, mouth.base,
          c.color('mouthBase'), meta: const {'part': 'expression'})
      ..addLayer('expression.mouth.light', 116, mouth.light,
          c.color('white'), meta: const {'part': 'expression'})
      ..addLayer('expression.brows.clear', 121, browClear,
          c.color('skinBase'), meta: const {'part': 'expression'})
      ..addLayer('expression.brows', 122, brows,
          c.color('hairShadow'), meta: const {'part': 'expression'})
      ..addLayer('expression.eyes.skin', 98, eyes.skin,
          c.color('skinBase'), meta: const {'part': 'expression'})
      ..addLayer('expression.eyes.dark', 99, eyes.dark,
          c.color('outline'), meta: const {'part': 'expression'})
      ..addLayer('expression.eyes.light', 100, eyes.light,
          c.color('white'), meta: const {'part': 'expression'})
      ..addLayer('expression.mark.dark', 123, marks.dark,
          c.color('skinDeep'), meta: const {'part': 'expressionMark'})
      ..addLayer('expression.mark.base', 124, marks.base,
          c.color('skinAccent'), meta: const {'part': 'expressionMark'})
      ..addLayer('expression.mark.light', 125, marks.light,
          c.color('fantasyLight'), meta: const {'part': 'expressionMark'});
  }

  String _effectiveEye(AvatarRenderContext c, String expression) {
    final explicit = c.string('v4.eyeExpression', 'auto');
    if (explicit != 'auto') return explicit;
    if (<String>['smile', 'bigSmile', 'laugh', 'openLaugh', 'blushingHappy'].contains(expression)) return 'happy';
    ...
    if (<String>['sleepy', 'tired', 'bored'].contains(expression)) return 'halfLidded';
    if (<String>['surprised', 'shocked', 'manic'].contains(expression)) return 'wide';
    if (<String>['suspicious', 'confident', 'mischievous', 'evilSmile'].contains(expression)) return 'suspicious';
    return 'neutral';
  }
  ...
  String _effectiveBrow(AvatarRenderContext c, String expression) {
    ...
    if (explicit != 'auto') return explicit;
    if (<String>['angry', 'furious', 'determined'].contains(expression)) return 'angryDown';
    if (<String>['sad', 'crying', 'worried'].contains(expression)) return 'sadUp';
    if (<String>['surprised', 'shocked'].contains(expression)) return 'surprisedHigh';
    if (<String>['suspicious', 'confident', 'mischievous', 'evilSmile'].contains(expression)) return 'skepticalSingle';
    if (<String>['sleepy', 'tired', 'bored'].contains(expression)) return 'sleepyFlat';
    return 'relaxed';
  }
  ...
  String _effectiveMouth(AvatarRenderContext c, String expression) {
    ...
    const mapping = <String, String>{
      'softSmile': 'smallSmile', 'smile': 'closedSmile',
      'bigSmile': 'wideSmile', 'grin': 'toothyGrin',
      'smirkLeft': 'smirkLeft', 'smirkRight': 'smirkRight',
      'laugh': 'laughOpen', 'openLaugh': 'laughWide',
      'mischievous': 'smirkLeft', 'angry': 'clenched',
      'furious': 'snarl', 'sad': 'sadFrown', 'worried': 'frown',
      'shy': 'tinySmile', 'surprised': 'oShape', 'shocked': 'shout',
      'suspicious': 'flatAnnoyed', 'confident': 'smirkRight',
      'proud': 'closedSmile', 'sleepy': 'breathingOpen',
      'tired': 'flatAnnoyed', 'bored': 'flatAnnoyed',
      'annoyed': 'grimace', 'determined': 'clenched',
      'evilSmile': 'fangSmile', 'manic': 'laughWide',
      'crying': 'sadFrown', 'blushingHappy': 'wideSmile',
      'disgusted': 'grimace',
    };
    return mapping[expression] ?? 'neutral';
  }

  String _animatedMouth(AvatarRenderContext c, String base, String animation,
      String motion) {
    ...
    if (animation == 'talk' || motion.startsWith('talk')) {
      final fast = motion == 'talkFast' ? 2 : motion == 'talkSmall' ? 5 : 3;
      return <String>['speakingM', 'speakingA', 'speakingE', 'speakingO'][(c.phase ~/ fast) % 4];
    }
  ...
  _MouthResult _mouth(AvatarRenderContext c, String style, int intensity) {
    ...
    if (style == 'tinySmile' || style == 'smallSmile') width = clampInt(width - 3, 3, 9);
    if (<String>['wideSmile', 'laughWide', 'shout'].contains(style)) width = clampInt(width + 2, 6, 15);
    ...
    if (style == 'neutral' || style == 'speakingM' || style == 'flatAnnoyed') {
    ...
    } else if (<String>['smallSmile', 'closedSmile', 'wideSmile', 'grin', 'toothyGrin'].contains(style)) {
      ...
      dark.line(left, y + (highLeft ? -1 : 1), 24, y + 1)
          .line(24, y + 1, right, y + (highLeft ? 1 : -1));
    } else if (style == 'laughOpen' || style == 'laughWide' ||
        style == 'openSmile' || style == 'shout') {
      final height = clampInt(open + (style == 'laughWide' || style == 'shout' ? 3 : 2), 2, 7);
      ...
      final inner = PixelMask()..fillEllipse(24, y, clampInt(radius - 1, 1, 4), clampInt(radius - 1, 1, 4));
      ...
      dark.fillTriangle((x: left + 2, y: y + 2), (x: right - 2, y: y + 2), (x: 24, y: y - 2));
  ...
  PixelMask _brows(AvatarRenderContext c, String style, int intensity) {
    ...
    void draw(int center, bool left) {
      if (style == 'surprisedHigh') {
        ...
        output.line(center - 3, y - (left ? 2 : 0), center + 3,
            y - (left ? 0 : 2), thickness: intensity >= 4 ? 2 : 1);
      } else if (style == 'sadUp' || style == 'liftedInner') {
        output.line(center - 3, y, center + 3, y - (left ? 2 : -2));
      } else if (style == 'liftedOuter') {
        output.line(center - 3, y - (left ? 2 : 0), center + 3, y - (left ? 0 : 2));
      } else if (style == 'arched' || style == 'mischiefCurve') {
        output.line(center - 3, y, center, y - 2 - high)
            .line(center, y - 2 - high, center + 3, y);
      } else if (style == 'confidentTilt') {
        output.line(center - 3, y + (left ? 1 : -1), center + 3, y + (left ? -1 : 1));
    ...
    if (browMotion == 'raiseLeft') {
      return output.subtract(maskRect(leftX - 4, y - 4, 9, 6))
          .union(PixelMask()..hLine(leftX - 3, leftX + 3, y - 3));
    }
    if (browMotion == 'raiseRight') {
      return output.subtract(maskRect(rightX - 4, y - 4, 9, 6))
          .union(PixelMask()..hLine(rightX - 3, rightX + 3, y - 3));
    }
    return output;
  }

  _EyeExpressionResult _eyes(AvatarRenderContext c, String style, int intensity) {
    ...
    final speed = c.integer('v4.expressionSpeed', 3);
    final cycle = animationPeriod(speed, slow: blink == 'slowBlink' ? 30 : 20,
        fast: blink == 'nervousBlink' ? 7 : 11);
    final step = positiveMod(c.phase + (blink == 'doubleBlink' ? positiveMod(c.phase, 5) : 0), cycle);
    final blinkNow = step >= cycle - 2;
    void one(int x, bool left) {
      var closeTop = 0;
      var closeBottom = 0;
      if (style == 'happy' || style == 'laughing') closeBottom = 1 + intensity ~/ 3;
      if (style == 'sleepy' || style == 'halfLidded') closeTop = 1 + intensity ~/ 3;
      if (style == 'narrowed' || style == 'angry' || style == 'determined') {
        closeTop = 1;
        closeBottom = 1;
      }
      final wink = (blink == 'winkLeft' && left) || (blink == 'winkRight' && !left);
      if (blinkNow || wink) {
        skin.fillRect(x - width ~/ 2 - 1, y - 3, width + 2, 7);
        dark.hLine(x - width ~/ 2, x + width ~/ 2, y);
        return;
      }
      if (closeTop > 0) skin.fillRect(x - width ~/ 2 - 1, y - 3, width + 2, closeTop + 2);
      if (closeBottom > 0) skin.fillRect(x - width ~/ 2 - 1, y + 1, width + 2, closeBottom + 2);
      ...
      if (style == 'suspicious') dark.hLine(x - width ~/ 2, x + width ~/ 2, y - (left ? 1 : 0));
    }
    one(leftX, true);
    one(rightX, false);
  ...
  _TripleMask _emotionMarks(AvatarRenderContext c, AvatarRenderState state,
      String expression) {
    ...
    final eyeY = c.integer('face.eyeY');
    if (style == 'blush') {
      base.data.setAll(0, orderedDither(state.mask('lowerCheekLeftZone')
          .union(state.mask('lowerCheekRightZone')), 4).data);
    } else if (style == 'tearLeft' || style == 'tearBoth') {
      light.line(c.integer('face.leftEyeX'), eyeY + 2,
          c.integer('face.leftEyeX') - 1, eyeY + 5 + c.integer('v4.tearAmount'));
      if (style == 'tearBoth') light.line(c.integer('face.rightEyeX'), eyeY + 2,
          c.integer('face.rightEyeX') + 1, eyeY + 5 + c.integer('v4.tearAmount'));
    } else if (style == 'sweatDrop') {
      light.fillEllipse(c.integer('head.rightX') - 1, eyeY - 3, 2, 3);
    } else if (style == 'angerMark') {
      dark.line(c.integer('head.rightX') - 4, eyeY - 7,
          c.integer('head.rightX'), eyeY - 3)
          .line(c.integer('head.rightX'), eyeY - 7,
              c.integer('head.rightX') - 4, eyeY - 3);
    } else if (style == 'stressLines') {
      for (var i = 0; i < 3; i++) dark.line(17 + i * 7, eyeY - 8, 18 + i * 7, eyeY - 4);
    } else if (style == 'sleepBubble') {
      light.fillEllipse(c.integer('head.rightX') + 3, c.integer('face.mouthY'), 2, 2)
          .fillEllipse(c.integer('head.rightX') + 6, c.integer('face.mouthY') - 4, 3, 3);
    } else if (style == 'sparkleMarks' || style == 'cheekStars') {
      for (final x in <int>[15, 33]) {
        light.set(x, eyeY + 3).set(x - 1, eyeY + 3).set(x + 1, eyeY + 3)
            .set(x, eyeY + 2).set(x, eyeY + 4);
      }
    } else if (style == 'heartMark') {
      base.set(34, eyeY - 5).set(36, eyeY - 5).hLine(34, 36, eyeY - 4).set(35, eyeY - 3);
    } else if (style == 'underEyeShadow') {
      dark.hLine(c.integer('face.leftEyeX') - 2, c.integer('face.leftEyeX') + 2, eyeY + 2)
          .hLine(c.integer('face.rightEyeX') - 2, c.integer('face.rightEyeX') + 2, eyeY + 2);
    } else if (style == 'magicFreckles') {
      for (var i = 0; i < 8; i++) light.set(16 + positiveMod(i * 7, 17), eyeY + 2 + positiveMod(i, 4));
    } else if (style == 'voidTears') {
      dark.line(c.integer('face.leftEyeX'), eyeY + 1, c.integer('face.leftEyeX'), eyeY + 7)
          .line(c.integer('face.rightEyeX'), eyeY + 1, c.integer('face.rightEyeX'), eyeY + 7);
    }
    return _TripleMask(dark, base, light);
  }
}
...
final class _TripleMask {
  const _TripleMask(this.dark, this.base, this.light)
      : accent = light;
  const _TripleMask.withAccent(this.dark, this.light, this.accent)
      : base = light;
