...
final class AnatomyRenderer implements AvatarPartRenderer {
  ...
  void render(AvatarRenderContext context, AvatarRenderState state) {
    ...
    state
      ..addLayer('torso.outline', 38, torso.torso.outline(diagonal: true),
          context.color('outline'), meta: const <String, Object?>{'part': 'torso'})
      ..addLayer('chest.skin', 39, torso.skinChest, context.color('skinBase'),
          meta: const <String, Object?>{'part': 'chest'})
      ..addLayer('clothing.base', 40, torso.clothing, context.color('clothBase'),
          meta: const <String, Object?>{'part': 'clothing'})
      ..addLayer('clothing.shadow', 41, clothShadow, context.color('clothDark'),
          meta: const <String, Object?>{'part': 'clothing'})
      ..addLayer('clothing.highlight', 42, clothLight, context.color('clothLight'),
          meta: const <String, Object?>{'part': 'clothing'})
      ..addLayer('clothing.pattern', 43, torso.pattern, context.color('clothAccent'),
          meta: const <String, Object?>{'part': 'clothing'})
      ..addLayer('clothing.seams', 44, torso.seams, context.color('clothDark'),
          meta: const <String, Object?>{'part': 'clothing'})
      ..addLayer('clothing.collar', 45, torso.collar, context.color('clothDark'),
          meta: const <String, Object?>{'part': 'clothing'})
      ..addLayer('neck.outline', 48, neck.outline(diagonal: true),
          context.color('outline'), meta: const <String, Object?>{'part': 'neck'})
      ...
      ..addLayer('ears.outline', 58, ears.outer.outline(diagonal: true),
          context.color('outline'), meta: const <String, Object?>{'part': 'ears'})
      ...
      ..addLayer('head.outline', 68, head.outline(diagonal: true),
          context.color('outline'), meta: const <String, Object?>{'part': 'head'})
      ...
      ..addLayer('head.highlight', 73, skinLight.subtract(skinShadow),
          context.color('skinLight'), meta: const <String, Object?>{'part': 'head'});
  }

  ({double top, double temple, double cheek, double cheek2, double jaw, double chin})
    ...
    switch (c.string('forehead.shape')) {
      case 'low': top += 2; temple += 1;
        break;
      case 'high': top -= 1;
        break;
      case 'veryHigh': top -= 2; temple -= 1;
        break;
      case 'wide': top += 2; temple += 2;
        break;
      case 'narrow': top -= 2; temple -= 1;
        break;
      case 'sloped': top -= 1;
        break;
      case 'rounded': top += c.integer('forehead.roundness') * 0.5;
        break;
    }
    ...
    double fit(double value) => clampInt(value.round(), 3, width + 1).toDouble();
    return (
      top: fit(top), temple: fit(temple), cheek: fit(cheek),
      cheek2: fit(cheek2), jaw: fit(jaw), chin: fit(chin),
    );
  }
  ...
  PixelMask _head(AvatarRenderContext c) {
    ...
    for (var y = top; y <= bottom; y++) {
      ...
      if (c.string('head.shape') == 'flatTop' && y <= top + 1) width = controls.top.round();
      if (c.string('forehead.shape') == 'sloped' && y < top + height * .22) {
        width -= ((top + height * .22 - y) * c.integer('forehead.slope') * .15).round();
      }
  ...
  PixelMask _neck(AvatarRenderContext c) {
    ...
    switch (c.string('neck.variant')) {
      case 'thin': topWidth -= 1; bottomWidth -= 1;
        break;
      case 'wide': topWidth += 2; bottomWidth += 2;
        break;
      case 'taperUp': topWidth -= 1; bottomWidth += 1;
        break;
      case 'flareDown': bottomWidth += 2;
        break;
      case 'asymmetric':
        break;
    }
    for (var y = top; y <= bottom; y++) {
      ...
      final width = lerpDouble(topWidth, bottomWidth + c.integer('neck.taper'), t).round();
  ...
  _TorsoResult _torso(AvatarRenderContext c) {
    ...
    var shoulderWidth = clampInt((c.integer('shoulders.width') + bodyDelta + massDelta).round(), 24, 48);
    var topWidth = clampInt((c.integer('torso.widthTop') + bodyDelta * .65 + massDelta).round(), 20, 48);
    var bottomWidth = clampInt((c.integer('torso.widthBottom') + c.integer('torso.taper') + bodyDelta * .75 + massDelta * .6).round(), 20, 48);
    final bodyType = c.string('body.type');
    if (<String>['massive', 'muscular', 'broad', 'shortWide'].contains(bodyType)) {
      shoulderWidth = clampInt((shoulderWidth + 2 + c.integer('body.mass') * .3).round(), 24, 48);
      topWidth = clampInt(topWidth + 2, 20, 48);
    }
    if (<String>['petite', 'verySlim', 'slim', 'tallNarrow'].contains(bodyType)) shoulderWidth -= 2;
    final torsoShape = c.string('torso.shape');
    if (torsoShape == 'wideChest') topWidth = topWidth < bottomWidth + 3 ? bottomWidth + 3 : topWidth;
    if (torsoShape == 'taperedDown') bottomWidth = clampInt(topWidth - 4 - clampInt(c.integer('torso.taper'), 0, 10), 20, 48);
    if (torsoShape == 'taperedUp') bottomWidth = clampInt(topWidth + 5 + clampInt(c.integer('torso.taper'), 0, 10), 20, 48);
    ...
    switch (c.string('shoulders.shape')) {
      case 'straight': slope = clampInt(slope, 0, 1);
        break;
      case 'sloping': slope += 2;
        break;
      case 'raised': slope = clampInt(slope - 2, 0, 20);
        break;
      case 'narrow': shoulderWidth -= 3;
        break;
      case 'broad': shoulderWidth += 3;
        break;
      case 'rounded': roundness += 2;
        break;
      case 'angular': roundness = 0; slope += 1;
        break;
      case 'muscular': shoulderWidth += 2; topWidth += 2;
        break;
      case 'delicate': shoulderWidth -= 2; roundness += 1;
        break;
    }
    ...
    for (var y = top; y <= 47; y++) {
      ...
      if (y <= top + 5) {
        final d = y - top;
        final shoulderInset = clampInt(d - leftDrop, 0, 20) + clampInt(d - rightDrop, 0, 20);
        final shoulderAt = shoulderWidth - clampInt(d * 2 - roundness, 0, 20) - (shoulderInset * .25).round();
        if (shoulderAt > width) width = shoulderAt;
      }
      if (torsoShape == 'rounded') width -= ((t - .5).abs() * (2 + roundness * .4)).round();
    ...
    if (armDepth > 0) {
      for (final direction in const <int>[-1, 1]) {
        final shoulderX = (center + direction * (shoulderWidth / 2 - 1)).round();
        for (var y = top + 2; y <= clampInt(top + 5 + armDepth * 2, 0, 47); y++) {
          final outward = ((y - top) * .35).round();
          final thickness = clampInt(((c.integer('body.mass') + armDepth) / 2).round(), 2, 8);
          arms.hLine(
    ...
    switch (c.string('clothing.neckline')) {
      case 'v':
        ...
        break;
      case 'square': necklineCut.fillRect(24 - neckWidth ~/ 2, top, neckWidth, necklineDepth + 3);
        break;
      case 'open': necklineCut.fillRect(24 - neckWidth ~/ 2, top, neckWidth, necklineDepth + 5);
        break;
      case 'asymmetric':
        necklineCut.fillTriangle(
          (x: 24 - neckWidth / 2, y: top),
      ...
      case 'high':
      case 'turtleneck':
      default: necklineCut.fillEllipse(23.5, top, neckWidth / 2, necklineDepth + 2);
        break;
    }
    final shoulderCoverage = clampInt(c.integer('clothing.shoulderCoverage'), 0, 5);
    ...
    skinChest.data.setAll(0, skinChest.union(necklineCut.intersect(fullTorso)).data);
    var clothing = fullTorso.subtract(skinChest);
    final garment = c.string('clothing.garment');
    if (garment == 'top') clothing = clothing.intersect(maskFromPredicate((x, y) => y >= top + 1));
    if (garment == 'hoodie') {
      final hood = PixelMask()..fillEllipse(23.5, top + 1, (neckWidth / 2 + 2).clamp(5, 20), 4);
    ...
    if (garment == 'jumpsuit') seams.vLine(24, top + 3, 47).hLine(18, 29, top + 5);
    ...
    } else if ((garment == 'shirt' || garment == 'coat') && c.integer('clothing.collarWidth') > 0) {
      final cw = c.integer('clothing.collarWidth');
      collar.fillTriangle((x: 24 - cw - 2, y: top), (x: 22, y: top + 4 + (cw / 2).ceil()), (x: 24 - cw + 1, y: top + 2));
      collar.fillTriangle((x: 24 + cw + 2, y: top), (x: 25, y: top + 4 + (cw / 2).ceil()), (x: 24 + cw - 1, y: top + 2));
    ...
    if (bounds != null) {
      if (patternStyle == 'stripe' || patternStyle == 'doubleStripe') {
        for (var y = bounds.y + 3; y <= bounds.bottom; y += density + 2) pattern.hLine(bounds.x, bounds.right, y);
      } else if (patternStyle == 'checker') {
        for (var y = bounds.y; y <= bounds.bottom; y++) {
          for (var x = bounds.x; x <= bounds.right; x++) {
            if ((((x - bounds.x) ~/ (density + 1)) + ((y - bounds.y) ~/ (density + 1))).isEven) pattern.set(x, y);
          }
        }
      } else if (patternStyle == 'dots') {
        for (var y = bounds.y + 2; y <= bounds.bottom; y += density + 2) {
          for (var x = bounds.x + 2; x < bounds.right - 1; x += density + 3) pattern.set(x, y);
        }
      } else if (patternStyle == 'diagonal') {
        for (var k = bounds.x - bounds.height; k < bounds.x + bounds.width + bounds.height; k += density + 3) {
          pattern.line(k, bounds.bottom, k + bounds.height, bounds.y);
        }
      } else if (patternStyle == 'sash') {
        pattern.line(bounds.x + 5, bounds.y, bounds.right - 6, bounds.bottom, thickness: 3);
      } else if (patternStyle == 'trim') {
        pattern.hLine(bounds.x, bounds.right, bounds.y + 3).vLine(24, bounds.y + 3, 47);
  ...
  _EarResult _ears(AvatarRenderContext c) {
    ...
    final animal = <String>['cat', 'fox', 'rabbit', 'bat'].contains(shape);
    if (animal) {
      final top = c.integer('head.topY');
      final baseY = top + 3;
      final width = c.integer('ears.width');
      final height = c.integer('ears.height') + c.integer('ears.tipLength');
      for (final side in const <int>[-1, 1]) {
        ...
        if (shape == 'rabbit') {
          ...
          outer.fillTriangle((x: cx - side * width / 2, y: baseY),
              (x: cx + side * (width + 2), y: top - height + 2),
              (x: cx + side * width / 2, y: baseY));
          outer.fillTriangle((x: cx, y: baseY - 1),
              (x: cx + side * (width + 4), y: top - height / 2),
              (x: cx + side * width / 2, y: baseY));
        } else {
          outer.fillTriangle((x: cx - side * width / 2, y: baseY),
              (x: cx, y: top - height + 2 - c.integer('ears.tipSharpness') * .25),
              (x: cx + side * width / 2, y: baseY));
        }
    ...
    for (final side in const <int>[-1, 1]) {
      final anchorX = side < 0 ? c.integer('head.leftX') : c.integer('head.rightX');
      final centerY = c.integer('ears.centerY') +
          (side < 0 ? clampInt(c.integer('ears.asymmetry'), 0, 5) : clampInt(-c.integer('ears.asymmetry'), 0, 5));
      var width = c.integer('ears.width');
      var height = c.integer('ears.height');
      var protrusion = c.integer('ears.protrusion');
      final isElf = shape.startsWith('elf') || <String>['goblin', 'fairy', 'demon', 'fin'].contains(shape);
      if (isElf) {
        final extra = shape == 'elfLong' ? 3 : (shape == 'elfMedium' ? 1 : 0);
        final length = c.integer('ears.tipLength') + extra;
        final tipX = anchorX + side * (width + protrusion + length);
        var tipY = centerY - c.integer('ears.angle') - (shape == 'elfUp' ? 4 : 0) + (shape == 'elfSide' ? 2 : 0);
        ...
        inner.line(anchorX + side, centerY, anchorX + side * (width + length - 1), tipY);
        ...
        inner.line(anchorX + side, centerY, anchorX + side * (width + protrusion), centerY)
            .vLine(anchorX + side * (width + protrusion), centerY - 1, centerY + 1);
      } else {
        if (shape == 'humanTiny') { width = 2; height = clampInt(height - 2, 3, 20); }
        if (shape == 'humanSmall') { width = clampInt(width - 1, 2, 20); height = clampInt(height - 1, 3, 20); }
        ...
        if (shape == 'humanRect') {
          outer.fillRect((cx - width / 2).round(), (centerY - height / 2 + shift).round(), width, height);
        } else {
          outer.fillEllipse(cx, centerY + shift, width / 2, height / 2);
        }
        outer.hLine(anchorX, anchorX + side * (protrusion + width * .35).round(), (centerY + shift).round());
        final detail = c.string('ears.innerDetail');
        if (detail != 'none') {
          if (detail == 'shell') {
            inner.fillEllipse(cx, centerY, (width / 4).clamp(1, 10), (height / 3).clamp(1, 10));
          } else {
            inner.line(anchorX + side, centerY - height ~/ 4,
                anchorX + side * clampInt(width - 1, 1, 20), centerY + height ~/ 4);
            if (detail == 'doubleLine') {
              inner.line(anchorX + side, centerY + 1,
                  anchorX + side * clampInt(width - 2, 1, 20), centerY - height ~/ 4);
            }
          }
        }
      }
