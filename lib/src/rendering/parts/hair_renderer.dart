  ...
  _FantasyResult _fantasy(
    ...
    if (hornStyle != 'none' && hornLength > 0) {
      void horn(int baseX, int direction, int extraLength) {
        ...
        final tipY = clampInt(topY - hornLength - extraLength, 0, 47);
        var tipX = baseX + direction * c.integer('fantasy.hornAngle');
      ...
      if (hornStyle == 'single') {
        ...
        horn(18, -1, asym < 0 ? 1 : 0);
        horn(30, 1, asym > 0 ? 1 : 0);
      }
    }

    final antenna = c.string('fantasy.antennaStyle');
    if (antenna != 'none') {
      final length = c.integer('fantasy.antennaLength');
    ...
    return _FantasyResult(back, front, marking.intersect(head.union(forehead)));
  }
