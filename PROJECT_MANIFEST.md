...
- Secure server: origin allowlist, remote-bind opt-in, bounded concurrency and token-gated atomic writes
...
- Result contract: frozen image, masks, palette, layout, graph and validation report
- Caches: canonical bounded LRU caches with hit/miss metrics and shared clearing
- Camera policy: exact phases with bounded representative sampling
...
- focused server routing, batch controller and atomic save repository;
- unit, contract and real HTTP integration tests;
- benchmark, CLI tools, release audit and catalog-generation tool.
